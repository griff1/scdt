 ...
 */
#ifndef SCDT_SERVER_HELPER_H
#define SCDT_SERVER_HELPER_H
...
public:
   ...
   */
  ScdtServerHelper (Address ip, uint16_t port);
  /**
   * Create ScdtServerHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
  ...
  /**
   * Given a pointer to a ScdtServer application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be ScdtServer).
   ...
   *
   * \param node The Ptr<Node> on which to create the ScdtServerApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the ScdtServerApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::ScdtServer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an ScdtServer will be installed.
